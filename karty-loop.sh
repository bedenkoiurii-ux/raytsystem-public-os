#!/usr/bin/env bash
# karty-loop.sh — БІЛЬШЕ НЕ ВЖИВАТИ. Замість нього: ./wl-loop.sh karty старт
#
# 2026-08-01: цей цикл не має жодного запобіжника — ні сну на ліміті, ні
# лічильника невдач, ні денного бюджету. Це рівно той клас, що 29.07 дав
# 594 холості прогони об weekly limit по вісім секунд двоє діб. Усе це вже
# є в спільному wl-loop.sh, тож karty переведено туди, а файл лишається
# заради дашборда (`karty-status.py`) і як запис, чому так.
#
# Старий опис нижче.
#
...
case "${1:-}" in
  ...
  лог|log) tail -f "$LOG" ;;
  дашборд|dashboard)                          # живий візуальний монітор у браузері
    exec python3 "$(dirname "$0")/karty-status.py" --open --watch ;;
  *) echo "вживання: $0 {старт|стоп|стан|лог|дашборд|раз}"; exit 1 ;;
esac
