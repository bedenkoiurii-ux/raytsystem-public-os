...
export function autolink(text: string, forms: Record<string, string>, proper?: Set<string>, surnames?: Set<string>, homonyms?: Record<string, Homonym>): string {
  ...
  // Недоторкані шматки — там, де розмітка ЗНАЧУЩА, а не оформлювальна.
  // Перевірка 03.08 знайшла чотири витоки поза цим списком: YAML-паспорт
  // (`title: Катинь` ставало посиланням, тобто мінявся зміст поля), код із
  // відступом у чотири пробіли, посилання на виноску (`[^Катинь]` ламалось
  // на `[^[[Катинь]]]`) і рядок таблиці (полагоджено окремо екрануванням).
  //
  // Коду з відступом у чотири пробіли тут НЕМАЄ навмисно. Замір 03.08:
  // у бібліотеці 1114 таких рядків, і кодом виглядають лише 337 (30%) —
  // решта це продовження списків і цитат. Сторож коштував би 2 900 підсвіток
  // заради трьохсот рядків, у яких назви сутностей майже не трапляються.
  // Справжній код тут завжди у потрійних лапках або в одинарних.
  //
  // Паспорт відрізається ОКРЕМО, не альтернативою в цій регулярці: із прапорцем
  // `m` вираз `^---…---` хапав будь-яку пару горизонтальних ліній посеред
  // документа й глушив цілі розділи — контроль показав мінус чотири тисячі
  // підсвіток. Паспорт має бути рівно один і рівно на початку.
  const guard = new RegExp("(" + [
    "```[\\s\\S]*?```",                    // блок коду
    "`[^`]*`",                             // inline-код
    "\\[\\[[^\\]]*\\]\\]",                    // вже наявний вікілінк
    "\\[\\^[^\\]\\n]+\\]",                     // посилання на виноску
    "\\[[^\\]]*\\]\\([^)]*\\)",                // [текст](адреса)
    "https?://\\S+",                       // гола адреса
    "^#{1,6} .*$",                         // заголовок
  ].map((x) => `(?:${x})`).join("|") + ")", "gm");
  // YAML-паспорт: рівно один, рівно на початку. Решта `---` — горизонтальні лінії.
  let head = "";
  let body = text;
  if (text.startsWith("---\n")) {
