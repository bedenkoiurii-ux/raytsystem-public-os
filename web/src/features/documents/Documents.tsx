...
export function Documents({ onShowInGraph, initialDocumentId }: DocumentsProps) {
  ...
  // Юрій (2026-09-10): у дереві теж має бути видно, який документ чекає
  // вердикту — не тільки в самому документі.
  const opponentCanonPaths = useMemo(
    () => new Set((opponentFindings.data?.items ?? []).map((item) => item.canon_path)),
    [opponentFindings.data]
  );
